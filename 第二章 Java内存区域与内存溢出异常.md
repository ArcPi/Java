...
2. 通过直接指针访问实例对象
   ...
   **无用的类的判定：**
   ...
   上述仅仅判断是的无用的类，是否进行回收还需要进行参数设置
   
   
   
   **什么时候需要回收永生代的垃圾回收：**
   
   在大量使用反射，动态代理，CGLib等ByteCode框架、动态生成JSP一级OSGi这类频繁的自定义ClassLoader的场景都需要虚拟机有类卸载功能，保证永生代不会溢出。3.
   ...
   **缺点：**
   ...
   将内存划分为两块，当这一块内存用完了，就把活着的对象复制按顺序复制到另外一块内存上去，然后把以前的那块内存全部清理掉，这样的问题是内存使用效率不高。
   
   
   
   现在的商业商业虚拟机都采用这个方式来回收新生代。98%的对象”朝生夕死“，所以将空间划分为一块较大的Eden空间和两块较小的survivor空间。每次只使用Eden和其中一块survivor空间。当回收时，把Eden和survivor中活着的对象复制到另一块survivor中，然后清理掉刚刚的Eden和survivor空间。
   
   
   
   ### 3.2.3 标记-整理算法
   
   标记完成后，让所有活着的对象向一端移动，然后直接清理掉边界以外的内存。
   
   
   
   ![截屏2020-07-15 下午8.45.58](第二章 Java内存区域与内存溢出异常.assets/截屏2020-07-15 下午8.45.58.png)
   
   ### 3.2.4 分代收集算法
   
   根据对象存活周期的不同将内存分为几块。一般把Java堆分为新生代和老年代，这样可以根据各个年代的特点采用不同的垃圾收集算法。
   
   
   
   **新生代：**对象存活率低，采用复制算法，且有老年代作为担保
   
   **老年代：**老年代中对象存活率高，且没有额外空间对它进行担保，所以使用标记-清理或者标记-整理算法来进行回收
   ...
   可达性分析算法中，查找引用链的时候，得保证分析工作在一个能保证一致性的快照中进行（就是系统不运转，停下，等待分析，这也是Java慢的一个原因），不可以在分析过程中对象引用还在不断变化的情况，该点不满足的话分析结果准确性就无法保证。这点是GC进行时必须停顿所有Java执行线程的一个重要原因。
   
   ### 3.3.2 安全点
   
   ## 3.4 垃圾收集器
   
   ![image-20200718160439642](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718160439642.png)
   
   **新生代：**Serial、ParNew、Parallel Scavenge
   
   **老年代：**CMS、Serial Old、Parallel Old
   
   **如果收集器之间有线连着，说明他们可以搭配使用。**
   ...
   1. 单线程收集器
   2. 在它进行垃圾收集时，必须暂停其他线程工作
   3. Client端一般采用这个模式
   
   ![image-20200718162834671](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718162834671.png)
   ...
   ![image-20200718203230341](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718203230341.png)
   
   ### 3.4.3 Parallel Scavenge收集器
   
   1. 达到一个可控制的吞吐量，(吞吐量 = 程序运行时间 / (程序运行时间 + 垃圾收集时间))
   ...
   ![image-20200718204703795](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718204703795.png)
   
   ### 3.4.5 Parallel Old收集器
   
   1. Parallel Old是Parallel Scavenge收集器的老年代版本
   
   ![image-20200718205337199](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718205337199.png)
   
   ### 3.4.6 CMS收集器(并发标记清除收集器)
   ...
   **初始标记、重新标记这两个步骤还是需要停顿，stop the world**
   
   初始标记仅仅标记GC Roots能直接关联到的对象，速度很快
   
   并发标记阶段就是进行GC Tracing的过程
   
   重新标记阶段是为了修正并发标记期间，因用户程序继续运转产生标记的对象发生变动的标记记录
   
   ![image-20200718210949158](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718210949158.png)
   
   ### 3.4.7 G1收集器
   
   **目标旨在避免全局垃圾收集**
   
   G1将整个Java堆(包括新生代、老年代)划分为多个大小固定的独立区域，并跟踪这些区域里的垃圾堆积程度，在后台维护一个优先列表，每次根据允许的收集时间，优先回收垃圾最多的区域。区域划分以及有优先级的区域回收，保证了G1收集器在有限的时间内获得最高的收集效率。
   ...
   对象在新生代Eden分配，当Eden没有足够空间分配的时候，就会发起一次Minor GC
   
   ### 3.5.2 大对象直接进入老年代
   
   ### 3.5.3 长期存活的对象进入老年代
   
   有个计数器，记录对象的年龄，每经过一次GC还活着的话，年龄加一，年龄达到15时，移到老年代
   
   ### 3.5.4 动态对象年龄判定
   
   ![image-20200718220024040](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718220024040.png)
   
   ### 3.5.5 空间分配担保
   
   ![image-20200718220527654](C:\Users\sheng\Desktop\workspace\面试知识点\Java\Java\第二章 Java内存区域与内存溢出异常.assets\image-20200718220527654.png)
